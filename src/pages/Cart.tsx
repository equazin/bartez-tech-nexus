...
import { AlertTriangle, ArrowLeft, Minus, Plus, Save, ShoppingCart, Upload } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] p-4 md:p-6">
      <div className="mx-auto max-w-[1400px] space-y-4">
        <div className="flex items-center justify-between rounded-xl border border-[#e5e5e5] bg-white px-4 py-3">
          <div className="flex items-center gap-3">
            <Link to="/b2b-portal" className="rounded-lg border border-[#e5e5e5] px-3 py-1.5 text-sm text-[#525252] hover:bg-[#f5f5f5]">
              <span className="inline-flex items-center gap-1"><ArrowLeft size={14} /> Volver</span>
            </Link>
            <h1 className="inline-flex items-center gap-2 text-lg font-bold text-[#171717]">
              <ShoppingCart size={18} /> Pedido mayorista
            </h1>
          </div>
          {isAdmin && (
            <div className="flex items-center gap-2 text-xs">
              <span className="text-[#737373]">Mínimo pedido</span>
              <input
                type="number"
                value={minOrderTotal}
                onChange={(e) => {
                ...
                className="w-28 rounded border border-[#e5e5e5] px-2 py-1 text-right"
              />
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[1.8fr_1fr]">
          <section className="rounded-xl border border-[#e5e5e5] bg-white">
            <div className="border-b border-[#f0f0f0] px-4 py-3 text-sm font-semibold text-[#171717]">
              Productos del pedido ({cartItems.length})
            </div>
            <div className="max-h-[540px] overflow-auto">
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-[#fafafa] text-[11px] uppercase text-[#737373]">
                ...
                <tbody>
                  {cartItems.map((item) => {
                    ...
                    return (
                      <tr key={item.product.id} className="border-t border-[#f0f0f0]">
                        <td className="px-3 py-2 font-mono text-[11px] text-[#525252]">{item.product.sku || "—"}</td>
                        <td className="px-3 py-2">
                          <div className="font-medium text-[#171717]">{item.product.name}</div>
                          <div className="mt-0.5 flex flex-wrap gap-2 text-[11px]">
                            <span className="text-[#737373]">Disp: {item.available} / Reservado: {item.product.stock_reserved ?? 0}</span>
                        ...
                        <td className="px-3 py-2">
                          <div className="mx-auto flex w-fit items-center gap-1">
                            <button className="rounded border border-[#e5e5e5] p-1" onClick={() => updateQty(item.product.id, item.quantity - 1)}><Minus size={12} /></button>
                            <span className="w-10 text-center font-semibold">{item.quantity}</span>
                            <button className="rounded border border-[#e5e5e5] p-1" onClick={() => updateQty(item.product.id, item.quantity + 1)}><Plus size={12} /></button>
                          </div>
                        </td>
                        <td className="px-3 py-2 text-right font-semibold text-[#171717]">{formatPrice(item.unitPrice)}</td>
                        <td className="px-3 py-2 text-right font-bold text-[#171717]">{formatPrice(item.totalWithIVA)}</td>
                      </tr>
                    );
                  })}
                  {!cartItems.length && (
                    <tr>
                      <td colSpan={5} className="px-3 py-10 text-center text-sm text-[#737373]">No hay productos en el carrito.</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          <section className="space-y-4">
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex items-center justify-between text-xs text-[#737373]">
                    ...
                    <input
                      ...
                      className="w-20 rounded border border-[#e5e5e5] px-2 py-1 text-right"
                    />
                  </div>
            ...
            <div className="rounded-xl border border-[#e5e5e5] bg-white p-4 space-y-3">
              <h3 className="text-sm font-semibold text-[#171717]">Condiciones de pago</h3>
              <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as (typeof paymentMethods)[number])} className="w-full rounded border border-[#e5e5e5] px-3 py-2 text-sm">
                {paymentMethods.map((m) => <option key={m} value={m}>{m}</option>)}
              </select>
              <div className="flex items-center justify-between text-xs text-[#737373]">
                <span>Recargo automático (%)</span>
                <input type="number" value={paymentSurchargePct} onChange={(e) => setPaymentSurchargePct(Math.max(0, Number(e.target.value) || 0))} className="w-20 rounded border border-[#e5e5e5] px-2 py-1 text-right" />
              </div>
            </div>

            <div className="rounded-xl border border-[#e5e5e5] bg-white p-4 space-y-3">
              <h3 className="text-sm font-semibold text-[#171717]">Logística / Envío</h3>
              <select value={shippingType} onChange={(e) => setShippingType(e.target.value as "Retiro en sucursal" | "Envío")} className="w-full rounded border border-[#e5e5e5] px-3 py-2 text-sm">
                <option>Retiro en sucursal</option>
                <option>Envío</option>
              </select>
              {shippingType === "Envío" && (
                <>
                  <input value={shippingAddress} onChange={(e) => setShippingAddress(e.target.value)} placeholder="Dirección de entrega" className="w-full rounded border border-[#e5e5e5] px-3 py-2 text-sm" />
                  <select value={shippingTransport} onChange={(e) => setShippingTransport(e.target.value as (typeof transportMethods)[number])} className="w-full rounded border border-[#e5e5e5] px-3 py-2 text-sm">
                    {transportMethods.map((m) => <option key={m}>{m}</option>)}
                  </select>
                  <div className="flex items-center justify-between text-xs text-[#737373]">
                    <span>Costo de envío</span>
                    <input type="number" value={shippingCost} onChange={(e) => setShippingCost(Math.max(0, Number(e.target.value) || 0))} className="w-24 rounded border border-[#e5e5e5] px-2 py-1 text-right" />
                  </div>
                </>
              )}
            </div>
            ...
            <div className="rounded-xl border border-[#e5e5e5] bg-white p-4 space-y-3">
              <h3 className="text-sm font-semibold text-[#171717]">Observaciones</h3>
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Notas del cliente, condiciones de entrega, aclaraciones..." className="w-full resize-y rounded border border-[#e5e5e5] px-3 py-2 text-sm" />
            </div>

            <div className="rounded-xl border border-[#e5e5e5] bg-white p-4 space-y-2">
              <h3 className="text-sm font-semibold text-[#171717]">Resumen final</h3>
              <div className="flex justify-between text-sm"><span className="text-[#737373]">Subtotal</span><span>{formatPrice(subtotal)}</span></div>
              <div className="flex justify-between text-sm"><span className="text-[#737373]">IVA</span><span>{formatPrice(ivaTotal)}</span></div>
              <div className="flex justify-between text-sm"><span className="text-[#737373]">Recargo pago</span><span>{formatPrice(surchargeAmount)}</span></div>
              <div className="flex justify-between text-sm"><span className="text-[#737373]">Envío</span><span>{formatPrice(shipping)}</span></div>
              <div className="border-t border-[#f0f0f0] pt-2 flex justify-between text-base font-bold"><span>Total final</span><span>{formatPrice(total)}</span></div>
              <div className="text-[11px] text-[#737373]">Moneda: {currency} · Cotización: {exchangeRate.rate.toLocaleString("es-AR")} ARS/USD · Ref: {currency === "USD" ? formatARS(total) : formatUSD(total)}</div>
            </div>
            ...
            <div className="space-y-2">
              <button onClick={generateQuote} className="w-full rounded-lg border border-[#e5e5e5] bg-white px-3 py-2 text-sm font-semibold text-[#171717] hover:bg-[#f5f5f5]">Generar cotización (PDF)</button>
              <div className="flex gap-2">
                <input value={saveListName} onChange={(e) => setSaveListName(e.target.value)} placeholder="Nombre de lista" className="flex-1 rounded border border-[#e5e5e5] px-3 py-2 text-sm" />
                <button onClick={saveAsList} className="inline-flex items-center gap-1 rounded-lg border border-[#e5e5e5] bg-white px-3 py-2 text-sm font-semibold text-[#171717] hover:bg-[#f5f5f5]"><Save size={14} /> Guardar</button>
              </div>
              <button
