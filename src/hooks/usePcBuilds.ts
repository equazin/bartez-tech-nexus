...
import type { PcBuildDraft, PcBuildItemDraft, PcBuildSlotKey, SavePcBuildDraftInput } from "@/models/pcBuilder";
...
const SUPPORTED_BUILD_SLOT_KEYS: PcBuildSlotKey[] = [
  "cpu",
  "motherboard",
  "ram",
  "gpu",
...
function isSupportedBuildSlotKey(value: string): value is PcBuildSlotKey {
  return SUPPORTED_BUILD_SLOT_KEYS.includes(value as PcBuildSlotKey);
}
...
function mapRowsToDrafts(buildRows: PcBuildRow[], itemRows: PcBuildItemRow[]): PcBuildDraft[] {
  ...
  for (const row of itemRows) {
    if (!isSupportedBuildSlotKey(row.slot_key)) {
      continue;
    }
