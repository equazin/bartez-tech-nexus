...
function SortIcon({ field, active, dir }: { field: string; active: boolean; dir: SortDir }) {
  ...
  return dir === "asc" ? <ArrowUp size={12} className="text-[#FF6A00]" /> : <ArrowDown size={12} className="text-[#FF6A00]" />;
}
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            className="w-full bg-[#232323] border border-[#2a2a2a] rounded-lg pl-8 pr-3 py-2 text-sm text-white outline-none focus:border-[#FF6A00]/50 placeholder:text-gray-600" />
        </div>

        {/* Category filter */}
        <select value={filterCat} onChange={(e) => setFilterCat(e.target.value)}
          className="bg-[#232323] border border-[#2a2a2a] rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#FF6A00]/50">
          ...
          <table className="w-full text-sm min-w-[900px]">
              ...
              <tr>
                <th className="px-3 py-3 w-8">
                  <input type="checkbox" checked={selected.size === filtered.length && filtered.length > 0}
                    onChange={toggleAll}
                    className="accent-[#FF6A00] w-3.5 h-3.5 cursor-pointer" />
                </th>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={p.id}
                    className={`border-t border-[#2a2a2a] transition ${isSelected ? "bg-[#FF6A00]/5" : "hover:bg-[#2a2a2a]/40"} ${!active ? "opacity-50" : ""}`}>
                    ...
                    <td className="px-3 py-3"
                      ...
                      {inlineCell?.id === p.id && inlineCell.field === "cost_price" ? (
                        <input ref={inlineRef} type="number" value={inlineValue}
                          ...
                          className="w-24 bg-[#181818] border border-[#FF6A00] rounded px-2 py-0.5 text-sm text-white outline-none" />
                      ) : (
                        <span className="text-white font-medium cursor-pointer hover:text-[#FF6A00] transition" title="Doble click para editar">
                    ...
                    <td className="px-3 py-3"
                      ...
                      {inlineCell?.id === p.id && inlineCell.field === "stock" ? (
                        <input ref={inlineRef} type="number" value={inlineValue}
                          ...
                          onBlur={commitInline}
                          onKeyDown={(e) => { if (e.key === "Enter") commitInline(); if (e.key === "Escape") setInlineCell(null); }}
                          className="w-16 bg-[#181818] border border-[#FF6A00] rounded px-2 py-0.5 text-sm text-white outline-none" />
                      ) : (
                    ...
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => duplicate(p)} title="Duplicar"
                          className="p-1.5 rounded-lg text-gray-500 hover:text-[#FF6A00] hover:bg-[#FF6A00]/10 transition">
                          <Copy size={13} />
                        </button>
