...
import {
  listRegistrationRequests as listRegistrationRequestsApi,
  updateRegistrationRequest as updateRegistrationRequestApi,
...
export function RegistrationRequestsTab() {
  ...
  async function fetchRequests() {
    ...
    try {
      const data = await listRegistrationRequestsApi("all");
      setRequests(data);
  ...
  async function updateStatus(
    ...
    setProcessing(id);
    try {
      const result = await updateRegistrationRequestApi(id, {
        status,
        ...(notes ? { notes } : {}),
        client_type: client_type as "mayorista" | "reseller" | "empresa" | undefined,
        default_margin,
      });
                ...
                {request.status === "pending" ? (
                  <div className="mt-4 space-y-4">
                    {onboardingForm?.id === request.id ? (
                      <div className="animate-in fade-in slide-in-from-top-2 space-y-4 rounded-2xl border border-primary/20 bg-primary/5 p-4">
                        <div className="flex flex-wrap items-center gap-4">
                          <div className="min-w-[200px] flex-1">
                            ...
                            <select
                              ...
                              className="h-9 w-full rounded-xl border border-border/70 bg-card px-3 text-xs font-semibold outline-none focus:border-primary/50 focus:ring-0"
                            >
