...
const PHASE_LABELS: Record<string, string> = {
  ...
  fetching:  "Descargando catálogo AIR...",
  upserting: "Guardando en Supabase...",
...
export function AirSyncTab({ isDark = true, userId, onSyncDone }: Props) {
  ...
  const label = `text-xs font-semibold uppercase tracking-widest mb-1 ${dk("text-gray-500", "text-[#a3a3a3]")}`;
  const big = `text-2xl font-extrabold tabular-nums ${dk("text-white", "text-[#171717]")}`;
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className={`font-bold text-base ${dk("text-white", "text-[#171717]")}`}>
            Sincronización AIR Intranet
          </h2>
          <p className={`text-xs mt-0.5 ${dk("text-gray-500", "text-[#737373]")}`}>
            api.air-intra.com · Lista 5 · Paginación 500 productos/página
          </p>
        </div>
        {lastSync && (
          <div className={`text-right text-xs ${dk("text-gray-500", "text-[#a3a3a3]")}`}>
            <p>Última sync: <span className="font-semibold">{lastSync.type === "catalog" ? "Catálogo" : "Precios/Stock"}</span></p>
            <p>{new Date(lastSync.finishedAt).toLocaleString("es-AR", { dateStyle: "short", timeStyle: "short" })}</p>
            <p className="text-[#2D9F6A]">+{lastSync.inserted} nuevos · ↻{lastSync.updated} actualizados</p>
          </div>
        )}
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className={card}>
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              <p className={`font-semibold text-sm ${dk("text-white", "text-[#171717]")}`}>Sync catálogo completo</p>
              <p className={`text-xs mt-0.5 ${dk("text-gray-500", "text-[#737373]")}`}>
                Baja todos los productos AIR. Los nuevos se insertan con nombre y categoría.
                Los existentes solo actualizan precio y stock.
              </p>
            </div>
          </div>
          <button
        ...
        <div className={card}>
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              ...
              <p className={`text-xs mt-0.5 ${dk("text-gray-500", "text-[#737373]")}`}>
                Solo actualiza precios y stock de productos ya cargados. Más rápido.
                No toca nombre ni categoría.
              </p>
            </div>
          </div>
          <button
          ...
          >
            <Zap size={14} />
            {running ? "Sincronizando..." : "Sync rápido precios/stock"}
          </button>
        </div>
      </div>
      ...
      {!isIdle && (
        <div className={card}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className={`text-sm font-semibold ${isError ? "text-red-400" : isDone ? "text-[#2D9F6A]" : dk("text-white", "text-[#171717]")}`}>
          ...
          {running && progress.fetched > 0 && (
            <div className={`h-1.5 rounded-full mb-4 overflow-hidden ${dk("bg-[#222]", "bg-[#e8e8e8]")}`}>
              <div
                ...
                style={{ width: `${Math.min(100, (progress.updated + progress.inserted) / Math.max(1, progress.fetched) * 100)}%` }}
              />
            </div>
          )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <Metric label="Descargados" value={progress.fetched} icon={Package} dk={dk} />
            ...
            <Metric label="Errores" value={progress.errors.length} color={progress.errors.length > 0 ? "text-red-400" : undefined} dk={dk} />
          </div>
      ...
      <div className={`rounded-xl px-4 py-3 text-xs border ${dk("bg-blue-500/8 border-blue-500/20 text-blue-300", "bg-blue-50 border-blue-200 text-blue-700")}`}>
        <p className="font-semibold mb-1">¿Cómo funciona el sync?</p>
        <ul className="space-y-0.5 list-disc list-inside">
          <li><strong>Productos nuevos</strong>: se insertan con nombre, categoría, precio e imagen de AIR.</li>
          <li><strong>Productos existentes</strong>: solo se actualiza precio y stock. <strong>Nombre y categoría editados desde el Admin no se sobreescriben.</strong></li>
          <li>Los cambios de precio quedan registrados en el historial de precios.</li>
        </ul>
      </div>
...
function Metric({
  label, value, color, icon: Icon, dk,
}: {
  label: string;
  value: number;
  color?: string;
