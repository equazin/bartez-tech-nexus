...
import registrationRequestsHandler from "./api/registration-requests";
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (
          ...
          pathname === "/api/manage-user" ||
          pathname === "/api/registration-requests"
        ) {
          try {
            ...
            } else if (pathname === "/api/manage-user") {
              await manageUserHandler(vercelReq as never, vercelRes as never);
            } else {
              await registrationRequestsHandler(vercelReq as never, vercelRes as never);
            }
            return;
          } catch (error) {
